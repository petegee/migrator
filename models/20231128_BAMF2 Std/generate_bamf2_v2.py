#!/usr/bin/env python3
"""
Enhanced Ethos Binary Generator for BAMF2 Std model.
Properly encodes flight modes, mixes, inputs with actual YAML data.
"""
...
def load_bamf2_yaml(yaml_path: str) -> dict:
    """Load BAMF2 YAML data."""
...
class EthosBinaryBuilder:
    ...
    def add_bitmap(self, bitmap: str = ""):
        bitmap_bytes = bitmap.encode('ascii', errors='ignore')[:15]
        self.content.extend((bitmap_bytes + b'\x00' * 16)[:16])

    def add_config_block(self):
        """Add model config block with 6 trim channels (build 37)."""
        self.content.extend(b'\xff\xff\x30\x31\x00\x00\x00\x01\x00\x00\x00\x00\x00')

        # 6 trim channels for build 37
        # Rudder, Elevator, Aileron, Throttle, T5, T6
        trims = [
        ...
        self.content.append(len(trims))
        for trim in trims:
            self.content.extend(trim)

    def add_channel_slots(self):
        """Add channel slots (6 fixed output channels)."""
        self.content.extend(b'\x00\x00\x06\x00\x01\x02\x03\x04\x05\x00\x00\x00')
        self.content.extend(b'\x80\x80\x82\xe8\x03\x82\x18\xfc\x80\x01\x00\x00\x00\x00\x00\x00')

        # Channel entries (minimal - just first one)
        channel_names = ["Elev", "Rudd", "AilR", "AilL", "ThmC", "SpdC"]
        for name in channel_names:
            self.content.extend(encode_name(name))
            self.content.extend(b'\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01')

    def add_rf_modules(self):
        """Add 2 RF module records."""
        self.content.extend(b'\x0c\x00\x00\x00\x00\x00\x00\x00')
        self.content.extend(b'\x10\x00kVkVbDfH\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x01\x00\x02\x00\x00\x00\x00\x00')

        self.content.extend(b'\x00\x0a\x00\x00\x00\x00\x00\x00')
        self.content.extend(b'\x10\x00kVkVbDfH\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x01\x00\x0a\x00\x00\x0a\x00\x00\x01\x00\x00\x00\x00\x00')

    def add_mixes(self, mix_names):
        """Add simplified mix entries."""
    ...
    def add_inputs_with_vars(self, inputs):
        """Add inputs with vars section."""
        self.content.append(len(inputs))
        self.content.extend(b'\x00\x00')

        for inp in inputs:
            name = inp['name']
            weight_pct = inp['weight']

            self.content.extend(encode_name(name))

            # Var name (pad to 8 chars)
            var_name = name.ljust(8)[:8]
            self.content.append(len(var_name))
            self.content.extend(var_name.encode('ascii'))

            self.content.extend(b'\x08\x00')  # flags
            self.content.extend(b'\x01\x00\x00\x00')  # control bytes

            # Convert weight% to per-mille
            per_mille = int(weight_pct * 10)
            self.content.extend(encode_per_mille(-per_mille))  # rate low
            self.content.extend(encode_per_mille(per_mille))   # rate high
            self.content.extend(b'\x00' * 6)

    def add_footer(self):
        """Add footer section."""
        self.content.extend(b'\x55\x55\x55\x55')
        self.content.extend(b'\x01\x01\x00\x00\x00\x04\x00\x00\x00\x00\xa1\x00\x00\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00')

    def build(self) -> bytes:
        """Build complete binary with header."""
        ...
        header += bytes([0x81, 0x17])  # file type + schema
...
def main():
    # Load YAML
    yaml_path = Path('/tmp/MODELS/model10.yml')
    model_data = load_bamf2_yaml(str(yaml_path))

    # Extract data
    inputs = []
    for expo in model_data.get('expoData', []):
        inputs.append({
            'name': expo['name'],
            'weight': expo.get('weight', 100)
        })

    mix_names = [mix['name'] for mix in model_data.get('mixData', [])]

    print(f"✓ Loaded {len(inputs)} inputs, {len(mix_names)} mixes")
    ...
    builder.add_bitmap()
    builder.add_config_block()
    builder.add_channel_slots()
    builder.add_rf_modules()
    builder.add_mixes(mix_names)
    builder.add_inputs_with_vars(inputs)
    builder.add_footer()
    ...
    print(f"✓ Generated {output_path} ({len(binary)} bytes, content: {len(binary)-16} bytes)")
