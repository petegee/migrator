# Reverse Engineer BAMF2 Std from EdgeTX to Ethos — Attempt 3
...
**Output filename:** `attempt-3.bin` ← you MUST use exactly this name
...
This is **Attempt 3** — you have access to lessons learned from prior attempts (if any) in `/home/pete/source/ethos/migrator/templates/mistakes-and-lessons.md`.
...
```bash
! node /home/pete/source/ethos/migrator/lib/test-model.js attempt-3.bin
```
...
**Check the output:**
- `attempt-3_test_report.json` — status (PASS/FAIL), diffs
- `attempt-3_diff.txt` — byte changes
- `attempt-3_validation.txt` — structure analysis
...
- [ ] Binary file written to `attempt-3.bin`
- [ ] File size > 100 bytes (sanity check)
- [ ] Run: `! node /home/pete/source/ethos/migrator/lib/test-model.js attempt-3.bin`
