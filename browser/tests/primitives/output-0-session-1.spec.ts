/**
 * output-0 session-1: CH1 "Elev" output channel configuration
 *
 * ETX limitData[0]:
 *   name: Elev, min: 350, max: 0, revert: 0, offset: 0, ppmCenter: -24,
 *   symetrical: 1, curve: 0
 *
 * ETX encoding (discovered this session):
 *   min (positive N) → Ethos Min = -(N/10)%   [0 = default -100%]
 *   max (negative N) → Ethos Max = (|N|/10)%  [0 = default +100%]
 *
 * So for CH1:
 *   Min: -35.0%  (increase from -100% default by 65 × 1% steps)
 ...
 *
 * Build strategy: rebuild-from-scratch (navigateCreateModelWizard), then add
 * the output config.  Upload is broken — it doesn't exit the wizard.
 *
 * Phases:
 *   1. Boot + Wizard
 *   2. Vars: Rudder, Elevat, ThmC
 *   3. Mixes: Elev, RSComp
 *   4. Outputs: CH1 config
 *   5. Download
 */
...
const SC_DIR =
  '/home/pete/Source/ethos/migrator/test-results/output-0-session-1';

// ── Helpers (same as hvar-0-session-1) ───────────────────────────────────────
...
// "Elev" for channel name: E(auto-caps) Shift l e v ENTER
...
test('output-0 session-1: rebuild + CH1 Elev output config', async ({ page }) => {
  ...
  // ── Name: "Elev" ─────────────────────────────────────────────────────────
  // bitmap y=196 = center of Name row value area (display y≈157); y=150 hits stats area
  await tapBitmap(page, 738, 196); await page.waitForTimeout(800);
  await snap('05-ch1-name-keyboard');

  // Clear existing name (up to 10 chars)
  for (let i = 0; i < 10; i++) {
    await touchBitmap(page, 680, 395); await page.waitForTimeout(100);
  }
  // Type "Elev"
  ...
  // ── Min: -35.0% (from -100% default: increase by 65 × 1% steps) ─────────
  // Tap Min in DEFAULT (unscrolled) view — bitmap 340 = display 273 = Min row
  await tapBitmap(page, 600, 340); await page.waitForTimeout(500);
  await snap('07-ch1-min-bar');

  // Step-up: > icon is at bitmap x≈390 (section boundary ≈420). bx=437 was
  // hitting the '-' button (section 420-575). Use bx=395 for the '>' icon.
  ...
  // ── PWM center: -24μs ────────────────────────────────────────────────────
  // Scroll to reveal PWM center (100px scroll: byStart=300, byEnd=200)
  await cdpTouchSwipeBitmap(page, 400, 300, 200);
  await page.waitForTimeout(600);
  await snap('09-ch1-scrolled');

  // In ONE-SCROLL state (after 100px swipe), content shifted ~99bx.
  // PWM center DEFAULT bitmap y≈512; after scroll: 512-99=413.
  // bx=300 hits Max row (286-313 range) → wrong field.
  await tapBitmap(page, 600, 413); await page.waitForTimeout(500); // PWM center field
  await snap('10-ch1-ppm-bar');

  // Decrement 24 times: 0 → -24 (step starts at 1μs for PWM center)
  for (let i = 0; i < 24; i++) {
  ...
  // ── Curve: 0 → none (default, no change) ─────────────────────────────────
