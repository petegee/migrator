...
**CRITICAL FINDING (session-7):** Touching the "Weight 100%" text in the Free mix editor
does NOT open a numeric control bar. Instead, it opens a **full-screen Action editor**.
...
```typescript
// Open the Action editor for weight adjustment
await touchBitmap(page, 480, 440);  // tap "Weight 100%" → opens Action editor (full screen)
await page.waitForTimeout(700);
// → Full-screen Action editor opens — weight must be adjusted within this editor
// → Editor content and weight adjustment flow NOT YET EXPLORED
```

**Weight adjustment flow is NOT confirmed.** The Action editor must be explored in a
future session to discover:
- Weight field location and interaction method
- How to set specific percentage (25%, 80%, etc.)
- How to return to the mix editor after changes
...
- **Non-default weight**: weight≠100% — tapping "Weight 100%" opens full-screen Action editor
  (session-7). The weight adjustment within the Action editor has NOT been explored.
...
Write your spec to:
  `/home/pete/Source/ethos/migrator/browser/tests/primitives/mix-1-session-8.spec.ts`

Run it with:
```bash
cd /home/pete/Source/ethos/migrator/browser
npx playwright test tests/primitives/mix-1-session-8.spec.ts --reporter=list --project=chromium
```
...
2. **Write** the Playwright spec at `/home/pete/Source/ethos/migrator/browser/tests/primitives/mix-1-session-8.spec.ts`
...
6. **Write** `/home/pete/Source/ethos/migrator/primitive-migration/sessions/BAMF2_Std/mix-1/session-8/result.txt`:
...
| File | Content |
|------|---------|
| `/home/pete/Source/ethos/migrator/primitive-migration/sessions/BAMF2_Std/mix-1/session-8/result.txt` | SUCCESS or LEARN: notes |
