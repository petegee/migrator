...
Status: EXISTS (785 bytes) — upload this before editing
...
// Step 3: CDP touch swipe UP (byStart=290→byEnd=130) = scroll DOWN ~3 items
// Reveals: Trims≈163, Channels≈212, Vars≈261, Gyro≈310, Trainer≈358
await cdpTouchSwipeBitmap(page, 320, 290, 130);

// Step 4: tap "Channels" at bitmap y≈212
await tapBitmap(page, 320, 212);
await page.waitForTimeout(700);
// → 2-col picker: Category col (Vars/Channels(orange)/Gyro) | Member col (CH1/CH2/...)

// Step 5: tap Member col row 3 to open Channel sub-list
await tapBitmap(page, 510, 309);
await page.waitForTimeout(700);
// → single-column sub-list of channel names

// Step 6: scroll sub-list if needed and tap target channel
// CH8 position estimated at bitmap y≈340 after sub-list scroll
await cdpTouchSwipeBitmap(page, 400, 290, 130);  // scroll down
await tapBitmap(page, 400, 340);                   // CH8 (estimated — UNCONFIRMED)
await page.waitForTimeout(700);
...
**Channel sub-list positions are UNCONFIRMED.** Need a session with vision API to read channel
names and verify exact tap coordinates for each channel number.

---

## Setting the Weight Field (non-default weight)

**CRITICAL FINDING (session-7, confirmed session-8):** Touching the "Weight 100%" text in
the Free mix editor does NOT open a numeric control bar. Instead, it opens a **full-screen
Action editor**.

The Actions row in the Free mix editor is at **bitmap y≈430-450** (display y≈345-360).
The "Weight 100%" text occupies **bitmap x≈375-495**.

### Step 1: Open the Action editor

```typescript
await touchBitmap(page, 480, 440);  // tap "Weight 100%" → opens Action editor (full screen)
await page.waitForTimeout(1000);
```

### Step 2: Action Editor Layout (session-8 pixel analysis)

The Action editor is a dark full-screen overlay (dark_ratio=0.92 vs mix editor's 0.81).
Content rows are in the upper portion (bitmap y≈75-250):

| Row | Bitmap y | Content |
|-----|----------|---------|
| 1 | ≈94 | Left-edge icon |
| 2 | ≈130-162 | **Weight value** (bright center text showing "100%") |
| 3 | ≈155 | Horizontal separator |
| 4 | ≈225-240 | Additional field |

### Step 3: Open the Weight Value Editor

Tap the **RIGHT side** of the weight value row to open the orange-bar overlay:

```typescript
await touchBitmap(page, 520, 162);  // right edge of weight row → orange-bar screen
await page.waitForTimeout(1000);
// → Orange-bar overlay appears (x≈250-547 bitmap, y≈141-345 bitmap)
// → This overlay shows the current weight value and likely provides adjustment controls
```

**Confirmed (session-8):**
- Tapping LEFT side of weight row → NO CHANGE (stays in Action editor)
- Tapping RIGHT edge (bitmap x≈480-520) of weight row → opens orange-bar overlay
- Double-tap on weight row → PARTIAL orange bar appears (compact editing state)
- Long-hold on weight row → NO CHANGE

**NOT YET CONFIRMED:** How to adjust the weight value within the orange-bar overlay.
The overlay has an orange-highlighted region (238×164 display px) showing the current
value, with text/labels below. The adjustment mechanism (keyboard, +/- buttons, slider)
...
## Adding a New Action (session-8)

Instead of editing the default "Always on Weight 100%" action, add a NEW action
via the "+ Add a new action" button at the bottom of the Free mix editor.

### Step 1: Focus and activate "+ Add a new action"

The button renders at the bottom edge of the editor (bitmap y≈452-479).
Regular `tapBitmap`/`touchBitmap` fail at the bottom edge. Use CDP wheel
to scroll the button into focus (it highlights orange), then CDP Enter to activate:

```typescript
// CDP wheel ×7 to focus the button (same pattern as Var editor's focusAddNewAction)
async function cdpWheelFocusAddNewAction(page, wheelCount = 7) {
...
await cdpEnterKey(page);   // activates the focused "+" button
// → New Action editor opens (45% diff from mix editor)
```

### Step 2: New Action Editor Layout (session-8)

The new action editor is a two-column overlay:
- **Left panel** (x=0-400): Action configuration fields
- **Right scrollbar** at x=425 (thin orange vertical line)
- **OK button** at bottom-center: bitmap y≈360, x≈278
- **Title**: "Action" at top

**NOT YET CONFIRMED:** How to select "Weight" action type and set rate to 25%
within the new action editor. Tapping at wrong positions dismisses the editor
(96%+ screen diff). The fields within the editor need precise coordinate mapping.
...
- **Placement popup with existing mixes**: y=187 places at first position (not last) when
  ...
  After mix-0 added (3 preset + 1 custom = 4 mixes), "Last position" estimated at y≈371
  (187 + 4×46) — UNCONFIRMED.
- **Non-default Operation**: mltpx=Multiply/Replace/Lock — untested.
- **Non-default weight**: weight≠100% — CONFIRMED flow to Action editor and orange-bar overlay
  (session-7, session-8). Actual weight value adjustment within orange-bar overlay NOT YET WORKING.
- **Non-default swtch**: switch condition pickup — untested.
- **Channel source (srcRaw: ch(N))**: Navigational flow confirmed (Channels → sub-list).
  Exact channel member positions unconfirmed. CH8 tap at (400, 340) is an estimate.
```
...
npx playwright test tests/primitives/mix-1-session-9.spec.ts --reporter=list --project=chromium
```
...
2. **Write** the Playwright spec at `/home/pete/Source/ethos/migrator/browser/tests/primitives/mix-1-session-9.spec.ts`
...
6. **Write** `/home/pete/Source/ethos/migrator/primitive-migration/sessions/BAMF2_Std/mix-1/session-9/result.txt`:
...
| File | Content |
|------|---------|
| `/home/pete/Source/ethos/migrator/primitive-migration/sessions/BAMF2_Std/mix-1/session-9/result.txt` | SUCCESS or LEARN: notes |
