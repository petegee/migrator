...
**Location of all emulator files:** `~/source/ethos/spike/`
...
| File | Purpose |
|------|---------|
| `X18RS_FCC.wasm` | Firmware binary (23 MB, Ethos X18RS FCC variant) |
| `X18RS_FCC_patched.js` | Emscripten JS wrapper — **use this one** (has patched exports) |
| `X18RS_FCC.js` | Original wrapper (less reliable for Node.js) |
| `wasm_radio.bin` | Radio settings file — **required** at init time |
| `test-model.js` | Main validation harness — run this for migration testing |
| `run-ethos.js` | Interactive runner for manual exploration |
| `probe-exports.js` | Enumerate all WASM exports |
...
node test-model.js ../migrator/models/<model>/attempt-N.bin
```
...
// value range: 0–4095 (12-bit ADC, centre = 2048)
...
M._setAnalogPosition(4, 2048);  // Pot/slider (index 4+)

// Equivalently with ccall:
M.ccall('setAnalogPosition', 'void', ['number','number'], [0, 4095]); // Aileron full right
```
